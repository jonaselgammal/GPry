...
- Bugfix (regression in v4.0): `length_scale_prior` for kernels was silently ignored.
- Bugfix: finite stand-in for `-inf` on UltraNest had wrong hard-coded default value.
- Bugfix: `SurrogateModel.predict` skipped the upper clipping when a batch was entirely masked, so the value returned for a point could depend on which other points shared its batch.
- Bugfix: `SurrogateModel` can now be built without an infinities classifier.
- Bugfix: Removed mutations of passed args in different places in `Runner` and `SurrogateModel`.
