...
To initialize it, we pass it the log-likelihood function as first argument, and the prior bounds as the second argument (or via the ``bounds`` keyword). More complicated prior specification can be passed by defining and passing as first argument a :cobaya:`Cobaya Model <models.html#model.Model>` (see :doc:`module_cobaya`).
