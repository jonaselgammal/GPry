...
This class implements an interface for the Bayesian inference code `Cobaya <https://cobaya.readthedocs.io>`_, by inheriting from Cobaya's :external+cobaya:class:`~sampler.Sampler` class.
...
If you prefer to control the calls to GPry but still use a Cobaya-defined model pipeline, you can pass a Cobaya :external+cobaya:class:`~model.Model` as the first argument of the :class:`~gpry.run.Runner`, and run GPry normally.
