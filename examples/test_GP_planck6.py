...
for param in model_info['params'].keys():
  has_ref = ("ref" in model_info['params'][param])
  if has_ref:
    #print(param,model_info['params'][param]['ref']['loc'],bf[counter],model_info['params'][param]['ref']['scale']*0.1)
    model_info['params'][param]['ref']['loc'] = bf[counter]
    model_info['params'][param]['ref']['scale']*=0.1
    counter+=1
    #print(param,model_info['params'][param]['ref'])
#print(model_info)
#quit()
...
if np.all(_check_checkpoint(cname)):
  ...
  _, gpr, _, _, _ = gpry.run.run(model, convergence_criterion="CorrectCounter", verbose=verbose,options={'max_accepted':2500, 'max_points':10000,'n_initial':50,'random_proposal_fraction':0.0,'zeta_scaling':1.1},callback=callback,checkpoint=cname,convergence_options={'n_correct':10,'threshold':0.01,'abstol':0.05})
#zeta_scaling=1.1
