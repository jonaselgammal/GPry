...
:meth:`gpry.surrogate.GaussianProcessRegressor.predict_mean_grad_batch` needs no chain
rule, and the training points ``surrogate.gpr.X_train_`` are natural seeds. Prior bounds
...
def _make_logp_grad(surrogate, beta):
    """
    ...
    """
    has_svm = getattr(surrogate, "infinities_classifier", None) is not None

    def logp_grad(U):
        U = np.atleast_2d(U)
        mean, grad = surrogate.predict_mean_grad_batch(U)
        logp = beta * mean
        grad = beta * grad
        if has_svm:
            try:
                finite = surrogate.infinities_classifier.is_finite_X(
...
def hmc_sample_gp_mean(
    ...
    seeds : array, shape=(n_chains, d), optional
        Explicit chain seeds (transformed space). Defaults to ``surrogate.gpr.X_train_``,
    ...
    if seeds is None:
        if not hasattr(surrogate.gpr, "X_train_"):
            raise ValueError("GPR is not fitted: no training points to seed HMC.")
        seeds = np.asarray(surrogate.gpr.X_train_, dtype=float)
    ...
    logp_grad = _make_logp_grad(surrogate, beta)
    logp, grad = logp_grad(U)

    n_eval_start = int(getattr(surrogate, "n_eval", 0))
    ...
    # --- Sampling ---
    n_eval_sampling_start = int(getattr(surrogate, "n_eval", 0))
    kept = []
    kept_logp = []
    accepts = []
    ...
    n_eval_end = int(getattr(surrogate, "n_eval", 0))
    return {
        "X": X,
        "logp": logp_out,
        "trace": trace,
...
def hmc_acquire(surrogate, bounds, rng=None, return_info=False, **hmc_kwargs):
    """
    ...
    **hmc_kwargs
    ...
    info = hmc_sample_gp_mean(surrogate, lo, hi, rng=rng, **hmc_kwargs)
    X_norm = info["X"]
...
def _build_jax_logdensity(surrogate, lo, hi, beta):
    """
...
def nuts_acquire(surrogate, bounds, rng=None, return_info=False, **nuts_kwargs):
    """
