...
from gpry.proposal import (
    InitialPointProposer,
    ReferenceProposer,
    PriorProposer,
    UniformProposer,
    MeanCovProposer,
...
class Runner:
    ...
    def __init__(
        ...
        if try_resuming:
            if mpi.is_main_process:
                ...
                if self.loaded_from_checkpoint:
                    ...
                    self.log(
                        "warning: Found checkpoint files but they were "
        ...
        # Truth, Surrogate, GPAcquisition, InitialProposer and ConvergenceCriterion
    ...
    def _construct_surrogate(self, surrogate):
        """Constructs or passes the surrogate model."""
        if isinstance(surrogate, SurrogateModel):
            ...
            raise TypeError(
                "'surrogate' should be a SurrogateModel instance, a dict of arguments for"
                " the SurrogateModel, or a string specifying the kernel ('RBF' or "
                "'Matern'). Got {surrogate}"
            )
    ...
    def _construct_gp_acquisition(self, gp_acquisition):
        ...
        if isinstance(gp_acquisition, GenericGPAcquisition):
            ...
            gp_acquisition_defaults = {
                "bounds": self.prior_bounds,
                "preprocessing_X": self.surrogate.preprocessing_X,
    ...
    def _construct_initial_proposer(self, initial_proposer):
        ...
        if isinstance(initial_proposer, InitialPointProposer):
            ...
            if propname_nosuffix == "reference":
                ...
                self.initial_proposer = UniformProposer(**initial_proposer_args)
            elif propname_nosuffix == "meancov":
                self.initial_proposer = MeanCovProposer(**initial_proposer_args)
            else:
                raise ValueError(
    ...
    def _construct_convergence_criterion(self, convergence_criterion, acq_has_mc=False):
        ...
        if convergence_criterion is None:
            convergence_criterion = {"RobustConvergence": {"policy": "s"}}
            if acq_has_mc:
        ...
        if (
            not isinstance(convergence_criterion, Sequence)
            and not isinstance(convergence_criterion, Mapping)
        ) or isinstance(convergence_criterion, str):
            convergence_criterion = [convergence_criterion]
        self.convergence = []
    ...
    def _construct_mc_options(self, mc_options):
        ...
        if mc_options is None:
            # Default to BlackJAX when JAX is available for full-JAX pipeline
            try:
    ...
    def _construct_options(self, options):
        ...
        _opt_or_default = lambda optname, default: (
            ...
            if options.get(optname, default) is not None
            else default
        )
    ...
    def gpr(self):
        """
        Deprecated. Use ``Runner.surrogate`` instead.
        """
        warnings.warn(
            "The `Runner.gpr` attribute will be deprecated in favour of "
            "`Runner.surrogate`. Please use that one instead from now on."
        )
    ...
    def logL(self, X):
        ...
        Always returns an array.
        """
        return self.surrogate.logp(X) - self.truth.logprior(X)
    ...
    def n_total_left(self):
        ...
        return self.max_total - self.surrogate.n_total

    @property
    def n_finite_left(self):
        """Number of truth evaluations with finite return value before stopping."""
        return self.max_finite - self.surrogate.n_regress
    ...
    def _share_surrogate(self, root=0):
        """
        Shares the surrogate model of the main process, restoring each process' RNG.
        """
        if not mpi.multiple_processes:
            return
        mpi.share_attr(self, "surrogate", root=root)
        self.surrogate.set_random_state(self.rng)
    ...
    def _share_convergence_from_main(self):
        ...
        if not mpi.is_main_process:
            self.convergence = [gpryconv.DummyMPIConvergeCriterion()] * len(
                mpi_awareness
            )
    ...
    def run(self):
        ...
        if not self.loaded_from_checkpoint:
            ...
            if mpi.is_main_process and self.verbose >= 4:
                ...
                print(self.surrogate.training_set_as_df(param_names=self.truth.params))
            # Check if any of the points in X_regress are close to each other
            if len(self.surrogate.X_regress) > 1:
                distances = np.linalg.norm(
                    self.surrogate.X_regress[:, None]
                    - self.surrogate.X_regress[None, :],
                    axis=-1,
                )
                if np.any(distances < 1e-10):
                    self.log(
                        "Warning: Some of the initial training points are very close "
        ...
        while (
            self.n_total_left > 0 and self.n_finite_left > 0 and not self.has_converged
        ):
            ...
            if mpi.is_main_process:
                ...
                self.banner(
                    ...
                    f"Total truth evals: {self.surrogate.n_total} "
                    f"({self.surrogate.n_regress} finite) of {self.max_total}"
                    + (
            ...
            self.old_surrogate = deepcopy(self.surrogate)
            self.progress.add_current_n_truth(
                self.surrogate.n_total, self.surrogate.n_regress
            )
            ...
            with TimerCounter(self.surrogate) as timer_acq:
                ...
                if len(y_pred) > 0:
                    in_training_set, duplicates = check_candidates(
                        self.surrogate, new_X
                    )
                    if mpi.is_main_process:
            ...
            if mpi.is_main_process:
                self.log(
                    f"[ACQUISITION] ({timer_acq.time:.2g} sec) Proposed {len(new_X)}"
                    " point(s) for truth evaluation.",
                    level=3,
                )
                self.log(
                    "New location(s) proposed, as [X, logp_gp(X), acq(X)]:", level=4
                )
            ...
            # falling back to random prior sampling.
            if len(y_pred) < max(1, self.n_points_per_acq // 2):
                ...
                if self.resamples > self.n_resamples_before_giveup:
                    if mpi.is_main_process:
                        self.log(
                            f"Acquisition exhausted after {self.resamples - 1} "
                            "re-tries. Falling back to random prior sampling.",
                            level=1,
                        )
                        fallback_to_random = True
                fallback_to_random = mpi.bcast(fallback_to_random)
                if fallback_to_random:
                    # Generate random points from the prior as fallback
                    if mpi.is_main_process:
                        n_random = self.n_points_per_acq
                        random_X = []
                        for _ in range(n_random):
                            X_candidate = self.initial_proposer.get(rng=self.rng)
                            while not is_in_bounds(
                                [X_candidate], self.prior_bounds, validate=False
                            )[0]:
                                X_candidate = self.initial_proposer.get(
                                    rng=self.rng
                                )
                            random_X.append(X_candidate)
                        new_X = np.array(random_X)
                        # Get GP predictions at these points for convergence tracking
                        y_pred = np.array([
                            float(self.surrogate.logp(x)) for x in new_X
                        ])
                        acq_vals = np.zeros(len(new_X))
                        self.log(
                            f"[RANDOM FALLBACK] Proposed {len(new_X)} random "
                            "point(s) from prior.",
                            level=3,
                        )
                    self.resamples = 0
            ...
            if self.callback:
                if mpi.is_main_process:
                    self.log("[CALLBACK] Calling callback function...", level=4)
                if self.callback_is_MPI_aware or mpi.is_main_process:
                    with Timer() as timer_callback:
                        self.callback(self)
                    if mpi.is_main_process:
                        self.log(
                            f"[CALLBACK] ({timer_callback.time:.2g} sec) Evaluated "
            ...
            with TimerCounter(self.surrogate, self.old_surrogate) as timer_convergence:
                self._check_convergence_parallel(new_X, new_y, y_pred)
                mpi.sync_processes()
            self.progress.add_convergence(
                timer_convergence.time,
                timer_convergence.evals,
            ...
            if mpi.is_main_process:
                ...
                if self.n_total_left <= 0:
                    lines += (
                        "- The maximum number of truth evaluations "
    ...
    def do_initial_training(self):
        """
        Draws an initial sample for the `surrogate` GP model until it has a training set of size
        ...
        if mpi.is_main_process:
            ...
            y_init = np.empty(0)
            if hasattr(self.surrogate, "y_regress"):
                if len(self.surrogate.y_regress) > 0:
                    pretrained = len(self.surrogate.y_regress)
                    X_init = self.surrogate.X_regress
                    y_init = self.surrogate.y_regress
        ...
        if n_to_sample_per_process == 0 and self.verbose > 1:  # Enough pre-training
            warnings.warn(
                "The number of pretrained points exceeds the number of initial samples"
            )
            return
        ...
        with Timer() as timer_truth:
            ...
            for i in range(n_iterations_before_giving_up):
                ...
                for j in range(n_to_sample_per_process):
                    ...
                    X_in_bounds = False
                    proposer_tries = 0
                    warn_multiple = 10 * self.surrogate.d
        ...
        if mpi.is_main_process:
            self.log(
                f"[EVALUATION] ({timer_truth.time:.2g} sec) "
        ...
        if mpi.is_main_process:
            # If the desired number of initial samples hasn't been reached,
            # proceed with what we have if we got at least 2 finite points.
            if not finished:
                if n_finite_new >= 2:
                    self.log(
                        f"Warning: Only {n_finite_new} of {n_still_needed} desired "
                        f"initial finite points found after {len(X_init)} evaluations. "
                        "Proceeding with available points.",
                        level=1,
                    )
                else:
                    raise RuntimeError(
                        f"Only {n_finite_new} finite initial sample(s) found after "
                        f"{len(X_init)} evaluations (need at least 2). Try increasing "
                        "'max_initial', or decreasing the volume of the prior."
                    )
            ...
            self.progress.add_fit(timer_fit.time, timer_fit.evals_loglike)
            n_starts = getattr(self.surrogate.gpr, "last_hyperopt_num_starts", None)
            requested = getattr(
                self.surrogate.gpr, "last_hyperopt_requested_restarts", None
            )
            if n_starts is not None:
                hyperopt_msg = f"Hyperparameters were optimized from {n_starts} start(s)"
                if requested is not None:
                    hyperopt_msg += f" (scheduled restarts: {requested})"
                hyperopt_msg += "."
            else:
                hyperopt_msg = (
    ...
    def _fit_surrogate_parallel(self, new_X, new_y, properties=None):
        ...
        if will_fit_hyperparams or mpi.is_main_process:
            if will_fit_hyperparams:
                if n_restarts_this_process == 1:
                    fit_msg = (
                        "Hyperparameters will be optimized from the scheduled start "
                        "plus deterministic data-driven starts."
                    )
                else:
                    fit_msg = (
                        f"Hyperparameters will use {n_restarts_this_process} scheduled "
                        "restart(s) plus deterministic data-driven starts."
                    )
            else:
                fit_msg = "Hyperparameters will be kept constant."
            self.log(
        ...
        best_i = lmls[np.argmax([lml for i, lml in lmls])][0]
        if mpi.is_main_process:
            self.log(
                f"[{mpi.RANK}] Overall best log-marginal-likelihood {lmls[best_i][1]}",
                level=4,
            )
        self._share_surrogate(root=best_i)
        msg = None
        if mpi.is_main_process:
            n_starts = getattr(self.surrogate.gpr, "last_hyperopt_num_starts", None)
            requested = getattr(
            ...
            if will_fit_hyperparams:
                if n_starts is not None:
                    hyperopt_msg = f"optimized from {n_starts} start(s)"
                    if requested is not None:
                        hyperopt_msg += f" (scheduled restarts: {n_restarts_total})"
                    hyperopt_msg += "."
                else:
                    hyperopt_msg = f"fit with {n_restarts_total} restart(s)."
            else:
                hyperopt_msg = "kept constant."
            msg = (
                ...
                f" regressor. Hyperparameters were {hyperopt_msg}"
            )
        return msg
    ...
    def set_fiducial_point(self, X, logpost=None, loglike=None):
        ...
        if len(X.shape) > 1 or len(X) != self.surrogate.d:
            raise TypeError(
                f"`X` appears not to have the right dimension: passed {X.shape} but "
                f"expected shape=({self.surrogate.d},)."
            )
        self.fiducial_X = X
    ...
    def set_fiducial_mc(self, X, logpost=None, loglike=None, weights=None):
        ...
        if self.surrogate.d == 1 and len(X) == 1:
            X = X.T  # corner case: input was a 1-d array in dim 1
        if X.shape[1] != self.surrogate.d:
            raise TypeError(
    ...
    def generate_mc_sample(
        ...
        _ns_sampler_names = {"nested"} | set(nsint._ns_interfaces.keys())
        if sampler_name.lower() in _ns_sampler_names:
            if resume:
                warnings.warn(
            ...
            X_mc, y_mc, w_mc = mc.mc_sample_from_gp_ns(
                ...
                params=self.params,
                sampler=sampler_name.lower() if sampler_name.lower() != "nested" else None,
    ...
    def diagnose_last_mc_sample(self):
        ...
        if mpi.is_main_process:
            ...
            mean_training, _ = mean_covmat_from_evals(
                self.surrogate.X_regress, self.surrogate.y_regress
            )
        ...
        if mpi.is_main_process:
            ...
            try:
                ...
                success &= (
                    kl_norm(mean_last_mc, cov_last_mc, mean_acq, cov_acq) < self.d
                )
        success = mpi.bcast(success if mpi.is_main_process else None)
        return success
    ...
    def plot_mc(
        self,
        samples_or_samples_folder=None,
        add_training=True,
        add_samples=None,
        output=None,
        output_dpi=200,
    ...
    def plot_distance_distribution(
        ...
        self.ensure_paths(plots=True)
        if output is None:
            output_1 = os.path.join(self.plots_path, f"Distance_distribution.{ext}")
        ...
        fig, ax = gpplt.plot_distance_distribution(
            self.surrogate, mean, covmat, density=False, show_added=show_added
        )
