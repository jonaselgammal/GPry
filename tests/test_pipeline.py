...
def test_pipeline(model, gpr="RBF", gp_acquisition="LogExp",
             convergence_criterion="CorrectCounter", callback=None,
             callback_is_MPI_aware=False, convergence_options=None, options={},
             checkpoint="files", load_checkpoint="overwrite", verbose=3, mc_sampler="mcmc",
             desired_kl=0.05):
    ...
    runner = Runner(model, gpr=gpr, gp_acquisition=gp_acquisition,
                 ...
                 checkpoint=checkpoint, load_checkpoint=load_checkpoint, verbose=verbose)
    # Call runner.run to test the BO loop
    runner.run()
    assert runner.has_run == True, "The run hasn't completed."
    assert runner.has_converged == True, "The run hasn't converged within the given number of samples."
    ...
    for i in range(len(x_values)):
        sample = dict(zip(list(model.parameterization.sampled_params()),np.array(x_values.iloc[i])))
